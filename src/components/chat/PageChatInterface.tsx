...
const PageChatInterface = ({ page, children }) => {
  ...
  useCopilotReadable({
    ...
    value: {
      ...
      contentLength: fullContent ? fullContent.length : (page?.content?.length || 0),
      extractionStatus: isExtracting ? 'extracting' : (fullContent ? 'complete' : 'unavailable')
    }
  });
  ...
  useCopilotAction({
    ...
    description: "Analyze and provide insights about the current page content using full extracted content",
    parameters: [
      {
        name: "analysisType",
        type: "string",
        description: "Type of analysis to perform: summary, keywords, topics, insights, or structure",
    ...
    handler: async ({ analysisType }) => {
      ...
      const contentSource = fullContent ? "full extracted content" : "stored content";
      ...
      switch (analysisType) {
        case "summary":
          ...
          return `Page Summary (based on ${contentSource}): "${pageData.title}" - ${pageData.description || "This page contains detailed content."} The content contains approximately ${wordCount} words and covers comprehensive information about the topic.`;
        
        case "keywords":
          if (!contentToAnalyze) {
            return "No content available for keyword extraction.";
          }
          const words = (contentToAnalyze + " " + pageData.title + " " + pageData.description)
            .toLowerCase()
            .replace(/[^\w\s]/g, ' ')
            .split(/\s+/)
        ...
        case "topics":
          return `Main topics in "${pageData.title}" (analyzed from ${contentSource}): Based on the ${fullContent ? 'comprehensive extracted' : 'available'} content, this page covers detailed information related to ${pageData.title}. ${fullContent ? 'Full content analysis provides deeper topic understanding.' : 'Limited to stored content - full extraction would provide more comprehensive topic analysis.'}`;
        ...
        case "insights":
          const insights = [
            ...
            fullContent ? '- Full content successfully extracted from live URL' : '- Using stored content (full extraction unavailable)',
            `- Analysis capability: ${fullContent ? 'Comprehensive' : 'Limited'}`
          ];
        ...
        case "structure":
          ...
          return `Content structure (${contentSource}): ${lines.length} lines, ${paragraphs.length} paragraphs. ${fullContent ? 'Full page structure analyzed including all elements.' : 'Structure based on stored content only.'}`;
  ...
  useCopilotAction({
    ...
    description: "Get detailed metadata information about the current page including extraction status",
    parameters: [],
    handler: async () => {
      return {
        id: page?.$id,
        ...
        extractionStatus: isExtracting ? 'in_progress' : (fullContent ? 'success' : 'failed'),
  ...
  useCopilotAction({
    ...
    handler: async () => {
      ...
      try {
        ...
        const data = await response.json();
        setFullContent(data.result?.raw_content || null);
        return `Successfully refreshed content from ${page.url}. New content length: ${data.result?.raw_content?.length || 0} characters.`;
      } catch (error) {
        setExtractionError(error.message);
...
- Content Length: ${fullContent ? `${fullContent.length} characters (full)` : `${page?.content?.length || 0} characters (stored)`}

Enhanced capabilities:
1. **Full Content Analysis**: ${fullContent ? "Complete page content extracted from live URL for comprehensive analysis" : "Using stored content - full extraction in progress or unavailable"}
2. **Deep Content Understanding**: Answer questions using ${fullContent ? "complete extracted content" : "available stored content"}
3. **Comprehensive Summaries**: Generate detailed summaries based on ${fullContent ? "full page content" : "stored content"}
4. **Advanced Topic Analysis**: Identify themes and topics from ${fullContent ? "complete content" : "available content"}
...
Content extraction status: ${isExtracting ? "Currently extracting full content from the live URL..." : fullContent ? "Full content successfully extracted and available for analysis" : "Using stored content - full extraction unavailable"}
...
Use the available actions like analyzePageContent(), getPageMetadata(), and refreshFullContent() to provide the most helpful assistance possible.`}
      labels={{
        ...
        initial: `Hi! I'm your enhanced AI assistant for "${page?.title || 'this page'}".\n ${fullContent ? 'I have access to the complete page content extracted from the live URL and can provide comprehensive analysis.' : isExtracting ? 'I\'m currently extracting the full page content to provide you with the best possible assistance.' : 'I\'m working with the stored content and will try to extract the full content for better analysis.'} What would you like to explore?`,
      }}
      className="copilot-sidebar-custom"
    >
      {typeof children === 'function' ? children({ extractionStatus: getExtractionStatus() }) : children}
    </CopilotSidebar>
  );
};
